  ...
  return <section className="card-elev overflow-hidden">
    ...
    <div className="space-y-4 p-5 sm:p-6">
      <form onSubmit={changePassword} className="rounded-2xl border border-white/[0.07] bg-[#0B0E18] p-4 sm:p-5">
        <div className="flex items-start gap-3"><span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-[#46C99A]/10 text-[#46C99A]"><KeyRound className="h-4 w-4" /></span><div><div className="text-sm font-medium">Changer le mot de passe</div><div className="mt-1 text-xs text-[#7E8798]">8 caractères minimum, avec une majuscule et un chiffre.</div></div></div>
      ...
      <div className="rounded-2xl border border-white/[0.07] bg-[#0B0E18] p-4 sm:p-5">
        <div className="flex items-start gap-3"><span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-[#4F8CFF]/10 text-[#8FB4FF]"><MonitorSmartphone className="h-4 w-4" /></span><div><div className="flex items-center gap-2 text-sm font-medium">Session actuelle <span className="rounded-full bg-[#46C99A]/10 px-2 py-0.5 text-pe-micro text-[#46C99A]">Active</span></div><div className="mt-1 text-xs text-[#7E8798]">Ce navigateur · dernière activité maintenant</div></div></div>
...
function AccountDataSettings({ user, exportAllData, exportingData }) {
  ...
  return <section className="card-elev overflow-hidden">
    ...
    <div className="space-y-4 p-5 sm:p-6">
      ...
      <div className="rounded-xl border border-[#46C99A]/15 bg-[#46C99A]/[0.04] p-3 text-xs leading-relaxed text-[#8FAE9D]">Format UTF-8 compatible Excel. Les données de démonstration et les secrets d’authentification ne sont jamais inclus.</div>
...
function BillingSettings({ user }) {
  ...
  return <div className="space-y-4">
    ...
    <section className="card-elev overflow-hidden glow-purple">
      ...
      <div className="grid gap-3 p-5 sm:grid-cols-2 sm:p-6">{enabled.map(f=><div key={f} className="flex items-center gap-2.5 rounded-xl border border-white/[0.06] bg-[#0B0E18] p-3 text-xs"><span className="grid h-5 w-5 place-items-center rounded-full bg-[#46C99A]/10"><Check className="h-3 w-3 text-[#46C99A]"/></span>{f}</div>)}</div>
