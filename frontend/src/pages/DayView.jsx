...
export default function DayView() {
  ...
  return <div className="pe-page pe-page-stack mx-auto max-w-[1800px]">
    ...
    {error && <div className="flex items-center justify-between gap-3 rounded-pe-xl border border-[#FF5252]/25 bg-[#FF5252]/10 p-4 text-sm text-[#FF8A8A]"><span>{error}</span><button onClick={load} className="inline-flex items-center gap-2"><RefreshCw className="h-4 w-4"/>Réessayer</button></div>}
    ...
    <div className="grid min-w-0 gap-4 xl:grid-cols-[minmax(0,1fr)_310px]">
      ...
      <aside className="pe-card h-fit xl:sticky xl:top-4">
        ...
        <div className="p-3"><div className="mb-2 grid grid-cols-7 gap-1 text-center text-[9px] font-semibold uppercase text-[#687288]">{["Lu","Ma","Me","Je","Ve","Sa","Di"].map(label => <span key={label}>{label}</span>)}</div><div className="grid grid-cols-7 gap-1">{cells.map(cell => { const active = cell.key === selectedDate; const hasTrades = cell.trades.length > 0; return <button key={cell.key} disabled={!cell.inMonth} onClick={() => chooseDate(cell.key)} className={`relative aspect-square rounded-md border text-[10px] transition ${!cell.inMonth ? "border-transparent opacity-20" : active ? "border-[#8067F4] bg-[#8067F4]/25 text-white" : hasTrades ? "border-[#4F8DFF]/30 bg-[#4F8DFF]/10 text-[#DDE5FF] hover:border-[#8067F4]/60" : "border-[#6571CF]/12 bg-[#090E1C] text-[#687288] hover:border-[#6571CF]/30"}`}><span>{cell.day}</span>{hasTrades && <i className={`absolute bottom-1 left-1/2 h-1 w-1 -translate-x-1/2 rounded-full ${cell.pnl >= 0 ? "bg-[#4F8DFF]" : "bg-[#D95C82]"}`}/>}</button>; })}</div></div>
      </aside>
    </div>
  </div>;
}
