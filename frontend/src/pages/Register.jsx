    ...
    <div className="relative z-10 w-full max-w-6xl grid lg:grid-cols-[1.05fr_.95fr] rounded-[28px] overflow-hidden border border-white/10 shadow-[0_30px_100px_rgba(0,0,0,.65)] bg-[#080A12]/90 backdrop-blur-xl">
      <section className="hidden lg:flex p-12 xl:p-16 flex-col justify-between relative overflow-hidden border-r border-white/5">
        <div className="absolute inset-0 opacity-70" style={{background:"radial-gradient(circle at 20% 20%,rgba(124,77,255,.28),transparent 48%),radial-gradient(circle at 80% 90%,rgba(79,140,255,.16),transparent 45%)"}}/>
          <div className="relative"><Logo size="lg"/><div className="mt-16 font-mono text-pe-micro uppercase tracking-[.24em] text-[#B58BFF]">Ton système de progression</div><h1 className="mt-5 text-5xl font-bold leading-[1.06] xl:text-6xl">Protège ton compte.<br/><span className="text-purple-grad">Comprends tes décisions.</span></h1><p className="mt-6 max-w-lg leading-relaxed text-[#9CA3AF]">Centralise tes comptes financés, mesure ta discipline et transforme chaque trade en une leçon exploitable.</p></div>
          <div className="relative my-12 grid grid-cols-3 gap-3">{[["94","Discipline"],["87%","Survie"],["5","Comptes"]].map(([v,l],i)=><div key={l} className="rounded-2xl border border-white/[0.07] bg-white/[0.035] p-4"><div className="font-mono text-xl font-bold" style={{color:["#B58BFF","#00E676","#4F8CFF"][i]}}>{v}</div><div className="mt-1 text-pe-micro text-[#6B7280]">{l}</div></div>)}</div>
      ...
      <section className="p-6 sm:p-10 lg:p-12 xl:p-14 bg-[#090B14]/85">
        ...
        <form onSubmit={submit} className="space-y-5 mt-8" aria-busy={loading}>
          ...
          <label className="block text-xs font-medium text-[#B5BBC9]">Mot de passe<div className="relative"><input type={showPassword?"text":"password"} required value={password} onChange={e=>setPassword(e.target.value)} placeholder="8 caractères minimum" data-testid="register-password" className={`${field} pr-12`}/><button type="button" onClick={()=>setShowPassword(v=>!v)} aria-label={showPassword?"Masquer le mot de passe":"Afficher le mot de passe"} className="absolute right-3 top-[18px] w-9 h-9 flex items-center justify-center text-[#6B7280] hover:text-white">{showPassword?<EyeOff className="w-4 h-4"/>:<Eye className="w-4 h-4"/>}</button></div></label>
              <div><div className="flex gap-2">{[0,1,2].map(i=><span key={i} className={`h-1 flex-1 rounded-full transition ${strength>i?["bg-[#FFB855]","bg-[#B58BFF]","bg-[#00E676]"][strength-1]:"bg-white/10"}`}/>)}</div><div className="mt-2 text-pe-micro text-[#6B7280]">Ajoute une majuscule et un chiffre pour renforcer le mot de passe.</div></div>
          ...
          <button type="submit" disabled={loading} className="btn-primary w-full py-3.5 inline-flex items-center justify-center gap-2 shadow-[0_12px_30px_rgba(124,77,255,.28)]" data-testid="register-submit">{loading?"Création…":<>Créer mon compte <ArrowRight className="w-4 h-4"/></>}</button>
            ...
            <div className="mt-5 flex items-center gap-2 text-xs text-[#6B7280]"><CheckCircle2 className="h-4 w-4 text-[#00E676]"/>Aucune carte bancaire requise.</div>
