...
export default function Register() {
  ...
  const submit=async(e)=>{e.preventDefault();if(password.length<6)return toast.error("Mot de passe min. 6 caractères");setLoading(true);try{const result=await register(email,password,name);if(result.requires_email_confirmation){toast.success("Vérifie ton e-mail pour activer ton compte.");nav("/login");return}toast.success("Compte créé. Configurons ton profil.");nav("/onboarding")}catch(err){toast.error(err.response?.data?.detail||"Inscription impossible")}finally{setLoading(false)}};
