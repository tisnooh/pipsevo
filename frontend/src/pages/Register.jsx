...
export default function Register() {
  ...
  const submit=async(e)=>{e.preventDefault();if(password.length<6)return toast.error("Mot de passe min. 6 caractères");setLoading(true);try{await register(email,password,name);toast.success("Compte créé. Configurons ton profil.");nav("/onboarding")}catch(err){toast.error(err.response?.data?.detail||"Inscription impossible")}finally{setLoading(false)}};
