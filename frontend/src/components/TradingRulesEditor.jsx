...
const NumberField = ({ label, hint, value, onChange, min, max, step = 1, suffix }) => (
  <label className="block text-xs text-[#9CA3AF]">
    ...
    <input type="number" min={min} max={max} step={step} value={value} onChange={e=>onChange(Number(e.target.value))} className="mt-2 w-full rounded-xl border border-white/10 bg-[#0D1020] px-4 py-3 font-mono text-white outline-none transition focus:border-[#7C4DFF] focus:ring-2 focus:ring-[#7C4DFF]/10"/>
...
const RuleToggle = ({ item, onToggle, onDelete, custom = false }) => (
  <div className="flex items-center gap-3 rounded-xl border border-white/[0.07] bg-[#0B0E18] p-3">
...
export default function TradingRulesEditor({ value, onChange }) {
  ...
  return <div className="space-y-6">
    ...
    <section className="rounded-2xl border border-white/[0.07] bg-white/[0.018] p-4 sm:p-5">
      ...
      <div className="mt-4 flex flex-col gap-2 sm:flex-row"><input value={draft} onChange={e=>setDraft(e.target.value)} onKeyDown={e=>{if(e.key==="Enter"){e.preventDefault();addCustom();}}} maxLength={120} placeholder="Écris une règle personnelle…" className="min-w-0 flex-1 rounded-xl border border-white/10 bg-[#0D1020] px-4 py-3 text-sm text-white outline-none focus:border-[#7C4DFF]"/><button type="button" onClick={addCustom} disabled={!draft.trim()} className="btn-ghost inline-flex items-center justify-center gap-2 disabled:cursor-not-allowed disabled:opacity-40"><Plus className="h-4 w-4"/>Ajouter</button></div>
