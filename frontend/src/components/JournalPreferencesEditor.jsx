...
export default function JournalPreferencesEditor({ value, onChange, checklist, onChecklistChange }) {
  ...
  return <div className="space-y-5">
    ...
    <div className="rounded-2xl border border-white/[0.07] bg-[#090C15] p-4 sm:p-5">
      ...
      <div className="mt-4 flex flex-col gap-2 sm:flex-row">{active==="instruments"&&<select value={market} onChange={e=>setMarket(e.target.value)} className="rounded-xl border border-[#6571CF]/20 bg-[#0C1122] px-3 py-2.5 text-xs text-white outline-none focus:border-[#8075ED]"><option value="futures">Futures</option><option value="cfd">CFD / Forex</option></select>}<input value={draft} onChange={e=>setDraft(e.target.value)} onKeyDown={e=>{if(e.key==="Enter"){e.preventDefault();add();}}} maxLength={100} placeholder="Ajouter une option…" className="min-w-0 flex-1 rounded-xl border border-[#6571CF]/20 bg-[#0C1122] px-4 py-2.5 text-sm outline-none focus:border-[#8075ED]"/><button type="button" onClick={add} disabled={!draft.trim()} className="btn-primary inline-flex w-full items-center justify-center gap-2 disabled:opacity-40 sm:w-auto"><Plus className="h-4 w-4"/>Ajouter</button></div>
