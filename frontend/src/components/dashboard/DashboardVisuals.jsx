...
export function DashboardKpiCard({ label, value, sub, sparkColor = "green", icon: Icon, testid, preview = false }) {
  ...
  return <Wrapper {...motionProps} className={`card-elev relative overflow-hidden group hover:border-white/15 ${preview ? "h-[116px] p-3" : "h-[160px] p-4 sm:h-[180px] sm:p-5"}`} data-testid={testid}>
    ...
    <div className={`pointer-events-none absolute inset-x-0 bottom-0 ${preview ? "h-[38px]" : "h-[58px]"}`}>{preview ? <svg viewBox="0 0 220 38" className="h-full w-full" preserveAspectRatio="none" aria-hidden="true"><defs><linearGradient id={gradientId} x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor={color} stopOpacity="0.5"/><stop offset="100%" stopColor={color} stopOpacity="0"/></linearGradient></defs><path d="M0 32 C32 34 40 25 70 26 S108 20 137 21 S181 11 220 8 L220 38 L0 38Z" fill={`url(#${gradientId})`}/><path d="M0 32 C32 34 40 25 70 26 S108 20 137 21 S181 11 220 8" fill="none" stroke={color} strokeWidth="2"/></svg> : <ResponsiveContainer width="100%" height="100%"><AreaChart data={sparkData(label.length, sparkColor === "red")} margin={{ top: 0, right: 0, left: 0, bottom: 0 }}><defs><linearGradient id={gradientId} x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor={color} stopOpacity="0.55" /><stop offset="100%" stopColor={color} stopOpacity="0" /></linearGradient></defs><Area type="monotone" dataKey="y" stroke={color} strokeWidth={2} fill={`url(#${gradientId})`} /></AreaChart></ResponsiveContainer>}</div>
  </Wrapper>;
}
