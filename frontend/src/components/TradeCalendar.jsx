  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        ...
        <Summary icon={monthPnl >= 0 ? TrendingUp : TrendingDown} label="Résultat net" value={money(monthPnl, { signDisplay: "always" })} color={monthPnl >= 0 ? "#00E676" : "#FF5252"} />
        <Summary icon={CalendarDays} label="Jours actifs" value={String(activeDays.length)} color="#B58BFF" />
        <Summary icon={TrendingUp} label="Jours positifs" value={`${winningDays}/${activeDays.length || 0}`} color="#00E676" sub={losingDays ? `${losingDays} jour${losingDays > 1 ? "s" : ""} négatif${losingDays > 1 ? "s" : ""}` : "Aucun jour négatif"} />
      </div>
          ...
          {cells.map(cell => {
            ...
            return (
              <button
                ...
                className={`relative min-h-[74px] border-b border-r border-white/[0.055] p-1.5 text-left transition sm:min-h-[108px] sm:p-3 ${!cell.inMonth ? "bg-black/20 text-[#373D49]" : "text-[#8992A2]"} ${hasTrades ? "cursor-pointer hover:bg-white/[0.025]" : "cursor-default"} ${active ? "z-10 bg-[#7C4DFF]/10 ring-1 ring-inset ring-[#7C4DFF]/60" : ""}`}
              >
                ...
                {hasTrades && <div className="mt-2 sm:mt-4"><div className={`truncate text-[10px] font-bold tabular-nums sm:text-sm ${positive ? "text-[#00DFA5]" : "text-[#FF6675]"}`}>{money(cell.pnl, { notation: "compact", maximumFractionDigits: 1, signDisplay: "always" })}</div><div className="mt-1 flex gap-1">{cell.trades.slice(0, 4).map((trade, index) => <span key={`${trade.id}-${index}`} className={`h-1.5 w-1.5 rounded-full ${Number(trade.pnl || 0) > 0 ? "bg-[#00DFA5]" : Number(trade.pnl || 0) < 0 ? "bg-[#FF6675]" : "bg-[#7C4DFF]"}`} />)}</div></div>}
              </button>
            );
          })}
        </div>
