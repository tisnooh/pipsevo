    ...
    <div className="product-preview-context product-preview-context--journal">
      ...
      <div className="product-preview-journal-layout">
        <section className="product-preview-card product-preview-journal-table">
          ...
          <div className="product-preview-journal-row is-head">{['DATE', 'ACTIF', 'SENS', 'SETUP', 'R', 'RÉSULTAT', 'DURÉE'].map((cell) => <span key={cell}>{cell}</span>)}</div>
          {JOURNAL_ROWS.map((row) => <div className="product-preview-journal-row" key={`${row[0]}-${row[1]}-${row[4]}`}>{row.map((cell, index) => <span className={(index === 2 || index === 4 || index === 5) ? (cell.includes('-') || cell === 'Vente' ? 'is-loss' : 'is-win') : ''} key={`${cell}-${index}`}>{cell}</span>)}</div>)}
        </section>
